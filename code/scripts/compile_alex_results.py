...
TPM_unexpressed_df = pd.read_csv("../../removed_genes_due_to_low_expression_threshold_1.tsv", sep='\t')

full_unexpressed_df = pd.merge(coverage_df, TPM_df)

def sigmoid(x):
    return 1/(1+np.power(np.e,-x))

asif_unexpressed_df = None
for i in range(full_unexpressed_df.shape[0]):
    curr_row = full_unexpressed_df.iloc[i, :]
    new_row = curr_row[:4].copy()
    coverages = [float(el) for el in curr_row["domain_coverage"][1:-1].split(", ")]
    impact_factor = 0
    for domain_coverage in coverages:
    ...
    for tpm in curr_row[4:]:
        tissue = curr_row.index[int(len(new_row)/2) + 2].split("_")[0]
        new_row = pd.concat([new_row, pd.Series({tissue+"_tpm": tpm, tissue+"_asif": impact_factor*tpm})])
    if asif_unexpressed_df is None:
        asif_unexpressed_df = pd.DataFrame([new_row])
    else:
        asif_unexpressed_df = pd.concat([asif_df, pd.DataFrame([new_row])])

asif_unexpressed_df.to_csv("../../PPI_ASIF_unexpressed_table.tsv", sep='\t', index=False)